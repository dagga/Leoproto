...
Ce document décrit les étapes pour compiler et lancer les parties back-office et front-office de l'application Leoproto,
ainsi que les URL d'accès.
...
## 1. Compilation et Lancement Global du Projet

### 1.1 Compilation Globale

Pour compiler les deux applications (back-office et front-office) en une seule commande :

1. **Ouvrez un terminal** à la racine de votre projet.
2. **Exécutez la commande de compilation globale :**
   ```bash
   ./gradlew buildAll
   ```
   (Sur Windows, utilisez `gradlew buildAll`)

   Cette commande va :
    * Compiler le back-office (Spring Boot).
    * Compiler le front-office (Next.js) pour la production.

### 1.2 Lancement Global (mode développement)

Pour lancer les deux applications (back-office et front-office) en mode développement avec une seule commande Gradle
...
3. Arrêtez-les en utilisant la commande `kill` :
   ...
   ```
   Vous pouvez également supprimer les fichiers `nohup.out` dans les répertoires `backend` et `frontend` si vous le
   souhaitez.

## 2. Gestion Individuelle des Composants

Si vous souhaitez gérer le back-office et le front-office séparément :

### 2.1 Back-office (Spring Boot)
...
3. Lancez l'application Spring Boot :
   ...
   L'application démarrera et écoutera généralement sur le port `8080`. Vous verrez des logs indiquant le démarrage de
   Spring Boot.
